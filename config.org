...
- [[#backup][BACKUP]]
- [[#beacon][BEACON]]
- [[#clipboard-on-wayland][CLIPBOARD ON WAYLAND]]
- [[#company][COMPANY]]
...
- [[#graphical-user-interface-tweaks][GRAPHICAL USER INTERFACE TWEAKS]]
  ...
  - [[#change-the-color-of-vc-mode][Change the color of vc-mode]]
...
(use-package general
  ...
  (dt/leader-keys
    ...
    "c y" '(clipboard-yank :wk "Clipboard yank"))
...
** Change the color of vc-mode

#+begin_src emacs-lisp
;; 1. Define your custom face
(defface my-custom-vc-face
  '((t (:foreground "cyan" :weight bold)))
  "Custom face for the Git branch in mode line.")

;; 2. Modify the mode line to use this face for vc-mode
;; We replace the default (vc-mode vc-mode) entry with a propertized version
(setq-default mode-line-format
              (mapcar (lambda (item)
                        (if (and (listp item) (eq (car item) 'vc-mode))
                            ;; Replace default vc-mode entry with our custom one
                            `(:eval (propertize (or vc-mode "") 'face 'my-custom-vc-face))
                          item))
                      mode-line-format))

;; 3. Force an update if Emacs is already running
...
** Lists
I want my lists to shine, to be noticeable
#+begin_src emacs-lisp
(font-lock-add-keywords 'org-mode
                        '(("^\\s-*\\([-]\\) " (1 '(:foreground "#1a9" :weight bold)))
                          ("^\\s-*\\([+]\\) " (1 '(:foreground "#b59" :weight bold)))))

(font-lock-add-keywords 'org-mode
			'(("^\\s-*\\([-+]\\) \\(\\[ \\]\\) " (1 '(:foreground "#0cb" :weight bold)) (2 '(:foreground "#dc3" :weight bold)))
			  ("^\\s-*\\([-+]\\) \\(\\[X\\]\\) " (1 '(:foreground "#0cb" :weight bold)) (2 '(:foreground "#558" :weight bold)))
			  ("^\\s-*\\([-+]\\) \\(\\[-\\]\\) " (1 '(:foreground "#0cb" :weight bold)) (2 '(:foreground "#07b" :weight bold)))))
...
#+RESULTS:
: [nil 26975 14246 603526 nil elpaca-process-queues nil nil 263000 nil]
